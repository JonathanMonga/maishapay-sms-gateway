...
public class PostMessage extends ProcessMessage {
            ...
            for (SyncUrl syncUrl : syncUrlList) {
                ...
                if (mPrefsFactory.enableWhitelist().get()) {
                    ...
                    for (Filter filter : filters) {
                        ...
                        if ((filter.getPhoneNumber().equals(message.getMessageFrom())) && (filter.getStatus().equals(Filter.Status.WHITELIST))) {
                ...
                if (mPrefsFactory.enableBlacklist().get()) {
                    ...
                    for (Filter filter : filters) {
                        ...
                        if ((filter.getPhoneNumber().equals(message.getMessageFrom())) && (!filter.getStatus().equals(Filter.Status.BLACKLIST))) {
                            if (postMessage(message, syncUrl)) {
                                postToSentBox(message);
                                deleteFromSmsInbox(message);
                            } else {
                ...
                if ((!mPrefsFactory.enableBlacklist().get()) && (!mPrefsFactory.enableWhitelist().get())) {
                    if (postMessage(message, syncUrl)) {
                        postToSentBox(message);
                        deleteFromSmsInbox(message);
                    } else {
    ...
    private boolean postMessage(Message message, SyncUrl syncUrl) {
        ...
        if (!TextUtils.isEmpty(syncUrl.getKeywords())
            ...
            List<String> keywords = new ArrayList<>(Arrays.asList(syncUrl.getKeywords().split(",")));
    ...
    private boolean postToWebService(Message message, SyncUrl syncUrl) {
        ...
        if (message.getMessageType().equals(Message.Type.PENDING)) {
            ...
            posted = mMessageHttpClient.postSmsToWebService(syncUrl, message, getPhoneNumber(), mPrefsFactory.uniqueId().get());
