...
public class TweetMessage extends ProcessMessage {
    ...
    private boolean tweetMessage(Message message) {
        ...
        if (!TextUtils.isEmpty(mPrefsFactory.twitterKeywords().get()) && mPrefsFactory.enableTwitterKeywords().get()) {
            List<String> keywords = new ArrayList<>(Arrays.asList(mPrefsFactory.twitterKeywords().get().split(",")));
